...
import Admissions.courses.Course;

public class AdminInterface {

	public void AllocateCourse() {
		try (SessionFactory sessionfac = new Configuration().configure()
				.addAnnotatedClass(StudentRegistration.class)
				.addAnnotatedClass(Admin.class)
				.buildSessionFactory();
				Session sess = sessionfac.openSession()) {

			List<StudentRegistration> StuList = sess
					.createQuery("FROM StudentRegistration ORDER BY Intermediate_Marks DESC", StudentRegistration.class)
					.list();
				...
				for (String priority : priorities) {
					...
					for (int i = 0; i < departments.length; i++) {
						if (priority.equalsIgnoreCase(departments[i]) && maxSeats[i] > Seatsalocate[i]) {
							...
							Seatsalocate[i]++;
							System.out.println(
									"Allocated seat in " + departments[i] + " for studentID" + Stud.getRegID());
	...
	public void Courses() {
		...
		try (SessionFactory sessionFactory = configuration.buildSessionFactory();
			...
			// Fetch data using Hibernate query (replace "Course" with your entity class
			// name)
			...
			System.out.println(
					"+----+------------+-------------------------------------------+------------+----------------+----------------+");
			System.out.println(
					"| id | courseCode | courseName                                | totalSeats | availableSeats | allocatedSeats |");
			System.out.println(
					"+----+------------+-------------------------------------------+------------+----------------+----------------+");
